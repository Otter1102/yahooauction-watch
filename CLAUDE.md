...
## 🚀 トライアル版・期限なし本番版 同時反映ルール（絶対）

- トライアル版 `https://yahooauction-watch-trial.vercel.app/` を修正したら、期限なし・買い切り本番版 `https://yahooauction-watch.vercel.app/` にも必ず同じ修正を反映し、両方をVercel productionへデプロイする。
- 期限なし・買い切り本番版を修正した場合も、トライアル版へ必ず同じ修正を反映し、両方をVercel productionへデプロイする。
- 片方だけのコード修正・片方だけのVercelデプロイで完了扱いにしない。環境差分が必要な場合は、その理由と差分を作業ログへ明記する。
