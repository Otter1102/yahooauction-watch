...
describe('通知送信の回帰防止', () => {
  ...
  it('GitHub Actions run-check もPush成功後だけ notified_items に保存する', () => {
    ...
    const deliveryCheck = source.indexOf('if (!delivered) {')
    const addHistory = source.indexOf('await addHistories(freshItems.map', deliveryCheck)
  ...
  it('条件保存後に全条件run-nowを自動で重ねて起動しない', () => {
    const source = readSource('app/page.tsx')
    const manualRefresh = source.indexOf('async function refreshLatestItems()')
    const runNowFetch = source.indexOf("fetch('/api/run-now'")

    expect(manualRefresh).toBeGreaterThan(-1)
    expect(runNowFetch).toBeGreaterThan(manualRefresh)
  ...
  it('定期チェック完了通知を環境変数で有効化できる', () => {
    ...
    expect(runCheck).toContain('successfulFetchByUser')
    expect(runCheck).toContain('allFetchesFailed')
    expect(runCheck).toContain('成功条件ありのため通常完了通知')
    ...
    expect(webpush).toContain('新着通知対象は0件です')
    expect(webpush).toContain('条件に合う開催中商品は0件でした')
    expect(webpush).toContain('警告: 取得エラーが発生しました')
  ...
  it('端末側のPush受信ACKと強制再登録が実装されている', () => {
    ...
    expect(page).toContain('RECENT_HISTORY_NOTICE_WINDOW_MS')
    expect(page).toContain('新着商品が見つかっています')
    expect(page).toContain('通知履歴に反映済み')
    expect(page).toContain('hasFetchedItemsWithoutPush')
    expect(page).toContain('!hasFetchedItemsWithoutPush')
  ...
  it('GitHub scheduleは毎時00分だけ起動し、バックアップworkflowは手動専用', () => {
    ...
    expect(workflow).toContain("cron: '0 0-15 * * *'")
    expect(workflow).not.toContain('7,22,37,52')
    expect(workflow).toContain("GH_FETCH_PAGES: '10'")
    expect(workflow).toContain("CHECK_SHARD_TOTAL: '12'")
    expect(workflow).toContain('CHECK_SHARD_INDEX: ${{ matrix.shard }}')
    expect(workflow).toContain("CHECK_FETCH_CONCURRENCY: '2'")
    expect(workflow).toContain("CHECK_SHARD_STAGGER_MS: '20000'")
  ...
  it('商品がなくても条件チェック履歴を残す', () => {
    ...
    expect(storage).toContain('条件チェック: 警告・取得エラー')
    ...
    expect(webpush).toContain('新着通知対象は0件です')
    expect(webpush).toContain('条件に合う開催中商品は0件でした')
    expect(webpush).toContain('ヤフオクwatch 要確認')
    expect(webpush).toContain('警告: 取得エラーが発生しました')
