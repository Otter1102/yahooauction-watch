...
| **残課題** | 追加対策後のGitHub Actions手動runで45分以内に完走するか確認する。完走ログで `notification_history` statement timeout が再発しないか確認する |
| **再発防止** | Actionsがsuccessでも巡回成功とは判断しない。`Run auction check` が実行されているか、`=== 完了` まで出ているか、`/api/health` が connected かを確認する。DB未応答時はfailureとして可視化し、大量履歴保存は小分けを維持する |
...
| **対策** | `notification_history` の物理削除を一時停止。履歴画面はサーバーが空を返しても端末キャッシュを消さず、`/api/history/restore` でキャッシュからDBへ復元する。Actions の Supabase precheck は service key で `conditions` を確認し、Node 22 / actions v5 / DB timeout 90秒へ更新。定期巡回は6 shard 並列へ分割し、shard 内も3並列取得、job timeout は30分へ拡張。チェック完了Pushは送信前に `notified_items` へ時間マーカーを予約して shard 間の重複を防止する |
| **解決した理由** | 巡回・手動更新・履歴取得の全経路で履歴を消さなくなり、端末に残った履歴キャッシュをDBへ戻せる。Actions 側は誤った anon key precheck と Node 20 警告を解消し、Supabase 遅延にも耐えやすくした。検索処理を6分割し、各 shard 内も3並列にしたため、ユニーク検索数が増えても1時間ごとの巡回が時間内に収まりやすい |
...
| 項目 | 内容 |
|------|------|
| **症状** | 条件作成時点で直近24時間以内に該当しそうなオークションがあっても、次の毎時チェックまで通知履歴が空のままになる |
| **原因** | 条件作成APIは `conditions` への保存だけを行い、保存直後の初回検索・履歴反映・取得完了通知を実行していなかった |
| **対策** | `lib/initial-check.ts` を追加し、`POST /api/conditions` の保存直後に同条件で検索。条件一致した現在出品中の商品を `notification_history` へ反映し、Web Pushで「取得完了しました」を送る |
| **解決した理由** | 条件作成直後に初回取得が走るため、毎時cronを待たずに履歴が埋まる。該当0件の場合も画面で「現時点の該当オークションはありません」と分かる |
| **再発防止** | `tests/unit/initial-check-regression.test.ts` で、条件作成APIが `runInitialConditionCheck()` を呼び、初回取得が履歴反映後に取得完了Pushを送ることを固定 |

---

## 2026-06-25 — Push失敗後に商品が通知済み扱いになり再通知されない

| 項目 | 内容 |
|------|------|
| **症状** | テスト通知は届くのに、実際の条件一致通知が来ない。前日にPush失敗した商品が次回以降も通知されない |
| **原因** | `/api/run-now` と `scripts/run-check.ts` が、Web Push送信成功前に `notified_items` へ保存していた。VAPIDキー整形エラー等でPush送信に失敗しても通知済み扱いになり、次回チェックでスキップされた |
| **対策** | Push/ntfy/Discord のいずれかが成功した後だけ `markNotified()` と `addHistory()` を実行。送信失敗時は `notified_items` に入れず次回再試行する |
| **解決した理由** | 通知配送に失敗した商品が通知済みとして固定されなくなり、次の自動チェックで再送対象に残る |
| **再発防止** | `tests/unit/notification-regression.test.ts` で、送信成功前に `notified_items` 保存へ戻らないことを固定 |
